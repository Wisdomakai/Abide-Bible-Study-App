...
export default function JoinGroupScreen({ navigation }) {
  ...
  const apply = async () => {
    ...
    setBusy(true);
    try {
      const result = picker.mode === 'create'
        ? await createGroup(picker.code, profile.name, picker.groupName || 'Bible Study')
        : { id: await joinGroupByCode(picker.code, profile.name) };
      const id = result.id;
      await refreshGroups();
