        ...
        renderItem={({ item }) => (
          <Card style={[styles.prayerCard, item.answered && styles.prayerCardDone]}>
            ...
            <View style={styles.prayerActions}>
              <Pressable disabled={sharingId === item.id} onPress={() => shareToGroup(item)} hitSlop={8} style={({ pressed }) => [sharingId === item.id && { opacity: 0.4 }, pressed && { opacity: 0.6 }]}>
                <Ionicons name="people-outline" size={19} color={colors.muted} />
              </Pressable>
              <Pressable
                onPress={() => confirmDestructive({
                  title: 'Remove prayer?',
                  message: 'This also removes every group copy you shared.',
                  confirmText: 'Remove',
                  onConfirm: async () => {
