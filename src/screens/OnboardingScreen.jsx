...
export default function OnboardingScreen() {
  ...
  const [picker, setPicker] = useState({ mode: 'create', code: '', groupName: '' });
  ...
  const finish = async () => {
    ...
    setBusy(true);
    try {
      const groupCode = picker.mode === 'create'
        ? (await createGroup(picker.code, name.trim(), picker.groupName || 'Our Bible Study')).code
        : picker.code;
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
        ...
        <View style={styles.footer}>
          {step === 1 ? (
            ...
            <>
              <Button title={busy ? 'Saving…' : 'Begin'} icon="checkmark" disabled={!picker.code || busy} onPress={finish} />
